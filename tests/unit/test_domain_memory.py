...
async def test_records_and_recalls_successful_stage():
    memory = DomainMemory(FakeRedis(), ttl_seconds=3600)
    await memory.record_success("example.com", "stage3_camoufox")
    assert await memory.get_last_successful_stage("example.com") == "stage3_camoufox"
...
async def test_domains_are_independent():
    ...
    await memory.record_success("b.com", "stage4_seleniumbase")
    assert await memory.get_last_successful_stage("a.com") == "stage1_curl_cffi"
    assert await memory.get_last_successful_stage("b.com") == "stage4_seleniumbase"
